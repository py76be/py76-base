...
PROJECT_NAME=py76-base
STACK_NAME=demo-py76-be
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://py76-base.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t py76be/py76-base-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t py76be/py76-base-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name py76-base-backend-acceptance -d py76be/py76-base-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name py76-base-frontend-acceptance --link py76-base-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d py76be/py76-base-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop py76-base-frontend-acceptance
	@docker stop py76-base-backend-acceptance
